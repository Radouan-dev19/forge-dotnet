...
- `/career` et `/career/{id}` : les cinq guides de carrière de la semaine 24 — CV par preuves, carnet STAR, prospection, négociation, plan 30/60/90 — aucune preuve de maîtrise, aucune promesse d'emploi ni de salaire ;
- `/ai` et `/ai/{id}` : le chapitre IA, hors parcours et en libre accès — modèle mental, économie de tokens, paramétrage, skills, agents et sous-agents, boucle quotidienne — aucune preuve de maîtrise ; les exercices comptés et les examens restent des séances sans IA ;
...
- SqlLab exige le profil Docker dédié (`scripts/start-sql-lab.ps1`) ; il est désactivé par défaut hors du profil `Development`, qui l'active pour le poste du développeur et affiche la procédure de démarrage sur `/sql-lab` quand le conteneur ne répond pas. SQL Server n'est jamais exposé directement sur le réseau hôte.
- En mode manuel, chaque page où l'effort serait perdu — `/practice`, `/projects`, `/debug-lab`, `/exams`, `/dashboard`, `/reviews`, `/mastery` — affiche la procédure d'activation du bac à sable ; un examen ne peut pas être démarré tant que l'installation ne peut pas le noter. Les laboratoires qui restaurent des paquets NuGet ou npm au premier lancement déclarent `requiresNetwork: true`. Les documents cités par les pages (`docs/HUMAN_REVIEW.md`, `docs/HUMAN_PANEL_KIT.md`, ce README) sont servis en lecture seule sur `/docs/{nom}`.
...
- La densité de pratique a atteint sa cible sur les semaines backend (mesure du 19 août 2026), et son état exact est figé par les matrices de `ContentS11S20CoverageTests` et `ContentS21S24CoverageTests` : S11 et S12 à onze exercices, S13 à dix, S14 à treize, S15 et S17 à huit, S16 et S19 à neuf, S18 à huit, S20 et S22 à six, S21 à neuf. S23 et S24 restent volontairement légères — deux et quatre exercices : leur charge vit dans le projet final et les preuves de défense et de carrière. Les semaines SQL S8 à S10 portent désormais chacune un exercice EF Core réel (`ef-*`) en plus du SqlLab. La couverture d'examen reste complète — les 214 exercices publiés figurent tous dans au moins une banque, la banque `delivery-pipeline-v1` couvrant S18–S20 — et le chantier T9 est clos : chacun des douze exercices à domaine d'entrée booléen relevés par l'audit forme une paire réciproque de `variantId` avec un frère à domaine ouvert sur le même sujet, propriété désormais figée par un test qui refuse tout `variantId` booléen vers booléen.

...
GROUP BY fixe le grain du résultat. Une sous-requête corrélée doit être examinée pour son coût et sa sémantique par ligne.

## Explication

GROUP BY fixe le grain du résultat. Une sous-requête corrélée doit être examinée pour son coût et sa sémantique par ligne. La règle doit rester visible dans le nom des opérations, les bornes et les erreurs. Avant de coder, notez l’entrée, la sortie, les invariants et ce qui doit être refusé.

## Exemple commenté

HAVING filtre les groupes après COUNT tandis que WHERE filtre les lignes avant regroupement. L’exemple est volontairement petit : changez une borne et une valeur absente pour vérifier que le raisonnement, et non la donnée mémorisée, produit le résultat.

## Contre-exemple et erreur fréquente

Sélectionner une colonne ni agrégée ni groupée produit une requête invalide ou ambiguë. Le contre-exemple doit être reproduit par un test qui échoue avant correction et réussit après.

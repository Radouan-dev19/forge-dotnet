...
Une pagination exige un ordre total. OFFSET convient à certains écrans ; une clé de continuation évite de rescanner un grand préfixe.

## Explication

Une pagination exige un ordre total. OFFSET convient à certains écrans ; une clé de continuation évite de rescanner un grand préfixe. La règle doit rester visible dans le nom des opérations, les bornes et les erreurs. Avant de coder, notez l’entrée, la sortie, les invariants et ce qui doit être refusé.

## Exemple commenté

Le couple OrderDate, OrderId départage les égalités et permet une pagination par clé stable. L’exemple est volontairement petit : changez une borne et une valeur absente pour vérifier que le raisonnement, et non la donnée mémorisée, produit le résultat.

## Contre-exemple et erreur fréquente

Paginer sans ORDER BY rend les pages non reproductibles et peut dupliquer ou perdre des lignes. Le contre-exemple doit être reproduit par un test qui échoue avant correction et réussit après.

# Managed Identity et Key Vault sans secret applicatif
...
Managed Identity donne une identité au service Azure ; Key Vault centralise les valeurs sensibles. Lʼapplication demande une valeur par identité et ne versionne ni valeur ni jeton.

## Explication

Managed Identity donne une identité au service Azure ; Key Vault centralise les valeurs sensibles. Lʼapplication demande une valeur par identité et ne versionne ni valeur ni jeton. Écrivez dʼabord le contrat, les entrées non fiables, la responsabilité exacte, les coûts possibles et la méthode de suppression. Une étape manuelle reste annoncée comme manuelle.

## Exemple commenté

La configuration contient lʼURI du coffre, non sensible, puis lʼidentité du service reçoit seulement Key Vault Secrets User sur ce coffre. Faites varier une contrainte et expliquez pourquoi le choix pourrait alors changer.

## Contre-exemple et erreur fréquente

Copier une valeur du coffre vers appsettings.json annule le bénéfice du mécanisme et laisse une trace durable. Reproduisez le défaut avec une preuve locale avant de proposer une correction.

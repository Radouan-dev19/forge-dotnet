...
La validation syntaxique et structurelle précède le métier. Les erreurs HTTP ont une forme stable et ne divulguent ni pile ni donnée sensible.

## Explication

La validation syntaxique et structurelle précède le métier. Les erreurs HTTP ont une forme stable et ne divulguent ni pile ni donnée sensible. Commencez par écrire le contrat, les entrées non fiables, la sortie observable et les limites de responsabilité. Une décision d’architecture n’est retenue que si elle réduit un risque ou rend une preuve plus directe.

## Exemple commenté

Une quantité hors plage reçoit 400 avec un type, un titre et les champs invalides ; un conflit métier reçoit 409. Modifiez ensuite une borne et un droit pour vérifier que le raisonnement, et non une valeur mémorisée, détermine le résultat.

## Contre-exemple et erreur fréquente

Attraper toute exception dans chaque action puis retourner son message expose des détails et duplique la politique. Reproduisez cette erreur dans un test avant de la corriger ; ne masquez ni exception ni code de sortie.

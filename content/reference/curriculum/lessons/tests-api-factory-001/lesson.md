...
Une fabrique démarre l’application en mémoire avec configuration factice et dépendances contrôlées. Le test parle HTTP plutôt que d’appeler directement le contrôleur.

## Explication

Une fabrique démarre l’application en mémoire avec configuration factice et dépendances contrôlées. Le test parle HTTP plutôt que d’appeler directement le contrôleur. Commencez par écrire le contrat, les entrées non fiables, la sortie observable et les limites de responsabilité. Une décision d’architecture n’est retenue que si elle réduit un risque ou rend une preuve plus directe.

## Exemple commenté

Le client anonyme obtient 401 sur POST, le mauvais rôle 403 et l’opérateur autorisé 201. Modifiez ensuite une borne et un droit pour vérifier que le raisonnement, et non une valeur mémorisée, détermine le résultat.

## Contre-exemple et erreur fréquente

Remplacer l’autorisation par un succès systématique dans tous les tests supprime précisément la preuve recherchée. Reproduisez cette erreur dans un test avant de la corriger ; ne masquez ni exception ni code de sortie.

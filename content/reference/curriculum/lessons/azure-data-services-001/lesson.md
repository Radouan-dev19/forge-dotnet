# Azure SQL et Storage avec moindre privilège
...
Azure SQL porte les données relationnelles ; Blob Storage porte les objets. Chaque accès utilise une identité dédiée et le droit minimal, sans identifiant réel dans le dépôt.

## Explication

Azure SQL porte les données relationnelles ; Blob Storage porte les objets. Chaque accès utilise une identité dédiée et le droit minimal, sans identifiant réel dans le dépôt. Écrivez dʼabord le contrat, les entrées non fiables, la responsabilité exacte, les coûts possibles et la méthode de suppression. Une étape manuelle reste annoncée comme manuelle.

## Exemple commenté

Le service web lit ses blobs avec un rôle de lecture et accède à sa base applicative, tandis que le stockage de progression Forge.NET reste local. Faites varier une contrainte et expliquez pourquoi le choix pourrait alors changer.

## Contre-exemple et erreur fréquente

Partager une clé de compte ou ouvrir la base à tout Internet remplace une frontière contrôlée par un secret durable. Reproduisez le défaut avec une preuve locale avant de proposer une correction.

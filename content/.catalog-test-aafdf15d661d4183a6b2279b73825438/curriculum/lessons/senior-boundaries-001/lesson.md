...
La frontière de sens se dit **bounded context** — le terme vient de la conception pilotée par le
domaine, **domain-driven design** —, le découpage raté se dit **distributed monolith**, et le refus
argumenté s'appuie sur le mot **coupling**. Pour l'inter-service, l'entretien attend un nom d'outil :
**gRPC**, le cadre d'appels de procédure distants sur HTTP/2 à contrats binaires, que l'industrie
oppose aux interfaces REST quand la latence et le typage des échanges internes comptent. Le citer en
une phrase — appels internes typés et rapides, REST pour les frontières publiques — suffit ; rien
dans ce parcours n'en dépend, et c'est voulu : le vocabulaire s'apprend ici, le geste ailleurs.

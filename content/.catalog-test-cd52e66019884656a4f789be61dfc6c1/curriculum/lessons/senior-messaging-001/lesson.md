...
La file des lettres mortes se dit **dead letter queue** — souvent DLQ à l'oral —, le message
empoisonné **poison message**, et l'ordre par clé s'adosse aux mots **partition** et **consumer
group**. Les deux outils que tout entretien de messagerie cite : **RabbitMQ**, le courtier à files
et à routage, où la DLQ est un échange dédié ; **Kafka**, le journal partitionné où l'ordre ne vaut
que par partition et où le retard d'un consommateur se dit consumer lag. Une phrase par outil suffit
en entretien — dire lequel est une file et lequel est un journal, et ce que cela change à l'ordre et
au rejeu. C'est du vocabulaire à connaître, pas une dépendance de ce parcours.

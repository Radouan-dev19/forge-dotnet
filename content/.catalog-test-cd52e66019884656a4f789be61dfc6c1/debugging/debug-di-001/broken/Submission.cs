...
public static class Submission
{
    ...
    {
        var services = new Dictionary<Type, Func<object>> { [typeof(IGreeting)] = () => new FormalGreeting() };
        return ((IGreeting)services[typeof(IGreeting)]()).Greet(name);
    }

    private interface IGreeting { string Greet(string name); }
    private sealed class FormalGreeting : IGreeting { public string Greet(string name) => $"Bienvenue {name.Trim()}"; }

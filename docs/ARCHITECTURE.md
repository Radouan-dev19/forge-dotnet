...
Limites mesurées : réseau `none`, utilisateur `1654:1654`, racine en lecture seule, `/workspace` en tmpfs 64 Mio, `/tmp` 16 Mio, 1 CPU, 512 Mio, 64 PID/threads, 256 fichiers ouverts, 25 s de compilation, 30 s globales de tests et sortie publique 64 Kio. Toutes les capabilities sont supprimées, `no-new-privileges` et `seccomp=builtin` sont imposés explicitement, aucun device/socket/secret n'est monté et le pilote de logs est `none`. La limite disque est le tmpfs : `fsize=64 Kio` empêchait CoreCLR de démarrer et a été écarté après mesure. Une concurrence globale de 1 à 4 (2 par défaut) protège la machine.

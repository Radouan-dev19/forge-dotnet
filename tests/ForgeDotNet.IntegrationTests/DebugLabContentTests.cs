...
{
    [Fact]
    public async Task ThirtyScenariosAreValidCompleteAndMappedToGenericRunnerSuites()
    {
        ...
        Assert.Equal(30, scenarios.Select(item => item.Id).Distinct(StringComparer.Ordinal).Count());
